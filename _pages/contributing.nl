...
Het is een goed idee lid te worden van onze <a href="http://lists.subsurface-divelog.org/cgi-bin/mailman/listinfo/subsurface">mailing lijst</a>. Deze lijst is in het Engels - ondanks het feit dat Subsurface en deze website zelf in vele talen beschikbaar is. Natuurlijk kunnen we ook prima opweg met "gebroken Engels" :-)
...
Hieronder staat een zeer beknopte samenvatting hoe software aanpassingen aan te leveren. Je kunt een zogenaamd <a href="https://github.com/Subsurface-divelog/subsurface/pulls">pull requests</a> op onze GitHub repository indieben of stuur ze als email naar de mailing lijst. Meer informatie over Git kan je vinden in de <a href="https://www.kernel.org/pub/software/scm/git/docs/user-manual.html">git user manual</a>.

Start met het ophalen vam de laatste software (zie <a href="https://subsurface-divelog.org/building/">Building Page</a> hoe dit te doen).
<code>cd subsurface
git checkout master
git pull</code>
...
Er opent een tekst editor (normaal gespoken kan je deze definieren met een omgevingsvariabele <tt>GIT_EDITOR</tt>. Hier voer je je beschrijving van de wijziging in. De eerste regel is de titel. Houd het kort en duidelijk. Dan volgt de feitelijke beschrijving; zie hieronder voor meer informatie.

Als je de commit wil aanpassen is "git commit --amend" de juiste manier. Voel je vrij je wijzigingen in meerdere kleinere delen op te slitsen. Dan zijn er de mogelijke vervolgstappen. Voor de mensen die gewend zijn met GitHub te werken, push je wijzigingen naar je eigen branch op GitHub en maak achtereenvolgens <a href="https://github.com/Subsurface-divelog/subsurface/pulls">pull request</a>. De 2e manier is via je lokale git respository. Draai: <code>git format-patch master..devel</code>
Dit maakt een aantal files met namen als 0001-Commit-title.patch, die je naar de mailing lijst stuurt.

Alle ingediende wijzigingen dienen een regel te bevatten:  "Signed-off-by: Name" aan het eind van de commit. Dit laat zien dat je de code zelf heft geschreven en dat je het recht hebt de wijziging als open source beschikbaar te stellen. Zonder signed-off wordt een wijziging niet geaccepteerd.

Zie: <a href="http://gerrit.googlecode.com/svn/documentation/2.0/user-signedoffby.html">Signed-off-by Lines</a>

Verder, schrijf goede commit berschrijvingen, zoals bijvoorbeeld (in Engels, de voertaal van het project):
